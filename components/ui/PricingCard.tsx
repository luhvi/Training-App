...
const PricingCard = ({ title, price, subscriptionInfo }: PricingCardProps) => {
  return (
    ...
    >
      <h1 className="mb-2 cursor-default text-2xl font-medium text-[oklch(0.95_0_0)]">
        {title === "premiumSubscription" ? "Premium" : "7-Day Free Trial"}
      </h1>
      <div className="mb-6">
        <span className="mr-2 cursor-default text-5xl font-bold text-[oklch(0.95_0_0)] drop-shadow-[0_0_4px_rgba(255,255,255,0.50)]">{`${title === "freeTrial" ? "Free" : "$" + price}`}</span>
        {title !== "freeTrial" ? (
